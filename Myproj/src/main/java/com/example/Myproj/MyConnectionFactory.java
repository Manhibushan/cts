...
public class MyConnectionFactory {
	...
	public Connection getMyConnection() throws IOException, SQLException
	{
		...
		prop.load(in);
		connection=DriverManager.getConnection(prop.getProperty("jdbc.connection.url"),prop.getProperty("jdbc.connection.user"),prop.getProperty("jdbc.connection.password"));
		return connection;
	}
